  ...
  jQuery.extend(true, _R, {
    ...
    animcompleted: function (_nc, opt) {
      ...
      if (t != undefined && t != "none")
        ...
        } else if (ap == "no1sttime") _nc.data("autoplay", "on");
    },
    ...
    animateTheCaptions: function (nextli, opt, recalled, mtl) {
      var base_offsetx =
          opt.sliderType === "carousel"
            ? 0
            : opt.width / 2 - (opt.gridwidth[opt.curWinRange] * opt.bw) / 2,
        base_offsety = 0,
      ...
      opt.layers["static"] =
        opt.layers["static"] ||
        opt.c.find(".tp-static-layers").find(".tp-caption");
      ...
      if (opt.debugMode) {
        ...
        nextli.append(
          '<div class="helpgrid" style="width:' +
            opt.gridwidth[opt.curWinRange] * opt.bw +
            "px;height:" +
            opt.gridheight[opt.curWinRange] * opt.bw +
            'px;"></div>'
        );
        var hg = nextli.find(".helpgrid");
        hg.append(
          '<div class="hginfo">Zoom:' +
            Math.round(opt.bw * 100) +
            "% &nbsp;&nbsp;&nbsp; Device Level:" +
            opt.curWinRange +
            "&nbsp;&nbsp;&nbsp; Grid Preset:" +
            opt.gridwidth[opt.curWinRange] +
            "x" +
	...
	***************************************/
    animateSingleCaption: function (
      ...
      if (
        opt.autoHeight == "on" ||
        (opt.minHeight != undefined && opt.minHeight > 0)
      )
      ...
      var handlecaption = 0,
        layervisible =
          makeArray(_nc.data("visibility"), opt)[opt.forcedWinRange] ||
          makeArray(_nc.data("visibility"), opt) ||
          "on";

      // HIDE CAPTION IF RESOLUTION IS TOO LOW
      if (
        layervisible == "off" ||
        (_gw < opt.hideCaptionAtLimit && _nc.data("captionhidden") == "on") ||
        _gw < opt.hideAllCaptionAtLimit
      )
        _nc.addClass("tp-hidden-caption");
      else _nc.removeClass("tp-hidden-caption");
      ...
      if (
        _nc.hasClass("tp-videolayer") &&
        _nc.data("videoposter") != undefined &&
        (_nc.data("posterOnMobile") == "on" ||
          _nc.data("posteronmobile") == "on") &&
        _ISM
      ) {
        var vidw =
            makeArray(_nc.data("videowidth"), opt)[opt.curWinRange] ||
            makeArray(_nc.data("videowidth"), opt) ||
            "auto",
          vidh =
            makeArray(_nc.data("videoheight"), opt)[opt.curWinRange] ||
            makeArray(_nc.data("videoheight"), opt) ||
            "auto";

        (vidw = parseFloat(ww)), (vidh = parseFloat(hh));

        _nc.append(
          '<div class="tp-videoposter" style="position:absolute;top:0px;left:0px;width:100%;height:100%;background-image:url(' +
            _nc.data("videoposter") +
            '); background-size:cover;background-position:center center;"></div>'
        );
        if (vidw != "100%")
          _nc.css({ minWidth: vidw + "px", minHeight: vidh + "px" });
        else _nc.css({ width: "100%", height: "100%" });
      ...
      if (_nc.find("img").length > 0) {
        ...
        var ww = im.data("ww"),
          ...
          ww =
            makeArray(im.data("ww"), opt)[opt.curWinRange] ||
            makeArray(im.data("ww"), opt) ||
            "auto",
          hh =
            makeArray(im.data("hh"), opt)[opt.curWinRange] ||
            makeArray(im.data("hh"), opt) ||
            "auto";
        ...
        if (_responsive !== "off") {
          if (_ba != "grid" && wful) im.width(ww);
          else im.width(ww * opt.bw);
          if (_ba != "grid" && hful) im.height(hh);
          else im.height(hh * opt.bh);
        } else {
          im.width(ww);
          im.height(hh);
        }
      ...
      if (
        ...
        if (!_nc.hasClass("fullscreenvideo") && !yvcover) {
          punchgs.TweenLite.set(_nc, {
            ...
            borderBottomWidth:
              Math.round(ncobj.borderBottomWidth * opt.bh) + "px",
            borderLeftWidth: Math.round(ncobj.borderLeftWidth * opt.bw) + "px",
            borderRightWidth:
              Math.round(ncobj.borderRightWidth * opt.bw) + "px",
            width: ww * opt.bw + "px",
            height: hh * opt.bh + "px",
          });
        } else {
      ...
      if (arrobj === undefined) {
        ...
        arrobj.voa =
          makeArray(_nc.data("voffset"), opt)[opt.curWinRange] ||
          makeArray(_nc.data("voffset"), opt)[0];
        arrobj.hoa =
          makeArray(_nc.data("hoffset"), opt)[opt.curWinRange] ||
          makeArray(_nc.data("hoffset"), opt)[0];
        arrobj.elx =
          makeArray(_nc.data("x"), opt)[opt.curWinRange] ||
          makeArray(_nc.data("x"), opt)[0];
        arrobj.ely =
          makeArray(_nc.data("y"), opt)[opt.curWinRange] ||
          makeArray(_nc.data("y"), opt)[0];
      }
      ...
      var vofs =
          _respoffset !== "off" ? parseInt(voa, 0) * opt.bw : parseInt(voa, 0),
        hofs =
          _respoffset !== "off" ? parseInt(hoa, 0) * opt.bw : parseInt(hoa, 0),
      ...
      var $lts = _nc.data("lasttriggerstate"),
        ...
        mdelay = triggerforce
          ? 0
          : $start === "bytrigger" || $start === "sliderenter"
          ? 0
      ...
      if (!triggerforce)
        if ($lts == "reset" && $start != "bytrigger") {
          ...
          $cts = "on";
        } else if ($lts == "reset" && $start == "bytrigger") {
      ...
      var $time = _nc.data("timeline") ? _nc.data("timeline").time() : 0,
        $progress =
          _nc.data("timeline") !== undefined
            ? _nc.data("timeline").progress()
            : 0,
        tl =
          _nc.data("timeline") ||
          new punchgs.TimelineLite({ smoothChildTiming: true });
      ...
      if (
        ($progress < 1 && _nc.data("outstarted") != 1) ||
        staticdirection == 2 ||
        triggerforce
      ) {
        var animobject = _nc;

        if (_nc.data("mySplitText") != undefined)
          _nc.data("mySplitText").revert();

        if (
          (_nc.data("splitin") != undefined &&
            _nc.data("splitin").match(/chars|words|lines/g)) ||
          (_nc.data("splitout") != undefined &&
            _nc.data("splitout").match(/chars|words|lines/g))
        ) {
          var splittarget = _nc.find("a").length > 0 ? _nc.find("a") : _nc;
          _nc.data(
            "mySplitText",
            new punchgs.SplitText(splittarget, {
        ...
        if (
          _nc.data("mySplitText") !== undefined &&
          _nc.data("splitin") &&
          _nc.data("splitin").match(/chars|words|lines/g)
        )
          animobject = _nc.data("mySplitText")[_nc.data("splitin")];

        var $a = new Object();

        var reverseanim =
          _nc.data("transform_in") != undefined
            ? _nc.data("transform_in").match(/\(R\)/gi)
            : false;
        ...
        if (!_nc.data("$anims") || triggerforce || reverseanim) {
          var $from = newAnimObject(),
            ...
            hashover =
              _nc.data("transform_hover") !== undefined ||
              _nc.data("style_hover") !== undefined;
          ...
          // DELAYS
          $from.elemdelay =
            _nc.data("elementdelay") == undefined
              ? 0
              : _nc.data("elementdelay");
          $result.anim.ease = $from.anim.ease =
            $from.anim.ease || punchgs.Power1.easeInOut;
            ...
            _nc.hover(
              function (e) {
                ...
                if (intl && intl.progress() == 1) {
                  if (
                    nc.data("newhoveranim") === undefined ||
                    nc.data("newhoveranim") === "none"
                  ) {
                    nc.data(
                      "newhoveranim",
                      punchgs.TweenLite.to(nc, t.speed, t.anim)
                    );
                  } else {
        ...
        if (
          ...
          if (
            $end != undefined &&
            (staticdirection == -1 || staticdirection == 2) &&
            $end !== "bytriger"
          )
            punchgs.TweenLite.delayedCall(
              parseInt(_nc.data("end"), 0) / 1000,
              _R.endMoveCaption,
              [_nc, _mw, _pw, opt]
            );
          else
        ...
        if (
          ($start != "sliderenter" ||
            ($start == "sliderenter" && opt.overcontainer)) &&
          (staticdirection == -1 ||
            staticdirection == 1 ||
            triggerforce ||
            (staticdirection == 0 &&
              $progress < 1 &&
              _nc.hasClass("rev-static-visbile")))
        )
          if (
      ...
      if (_nc.data("loopanimation") == "on")
        punchgs.TweenLite.set(_lw, { minWidth: eow, minHeight: eoh });

      if (
        _nc.data("slidelink") != 0 &&
        (_nc.data("slidelink") == 1 || _nc.hasClass("slidelink"))
      ) {
    ...
    endMoveCaption: function (_nc, _mw, _pw, opt) {
      ...
      if (_nc.data("timeline")) _nc.data("timeline").pause();
      else if (_nc.data("_pw") === undefined) return;
      ...
      var tl = new punchgs.TimelineLite(),
        ...
        $to = _nc.data("transform_out")
          ? getAnimDatas(
              newEndAnimObject(),
              _nc.data("transform_out"),
              opt.sdir == 1
            )
          : getAnimDatas(
              newEndAnimObject(),
        ...
        animobject =
          _nc.data("splitout") &&
          _nc.data("splitout").match(/words|chars|lines/g)
            ? _nc.data("mySplitText")[_nc.data("splitout")]
            : _nc,
        elemdelay =
          _nc.data("endelementdelay") == undefined
            ? 0
            : _nc.data("endelementdelay"),
        iw = _nc.innerWidth(),
        ih = _nc.innerHeight(),
        p = _pw.position();
      ...
      if (
        _nc.data("transform_out") &&
        _nc.data("transform_out").match(/auto:auto/g)
      ) {
        $from.speed = $to.speed;
      ...
      $to.anim.y =
        $to.anim.y * opt.bw ||
        getBorderDirections($to.anim.y, opt, iw, ih, p.top, p.left, "vertical");
      ...
      if ($mask_to) {
        ...
        tl.add(
          newmasktl.set(
            _mw,
            { overflow: "visible", overwrite: "auto" },
            elemdelay
          ),
          0
        );
...
**********************************************************************************************/
  ...
  var getBorderDirections = function (x, o, w, h, top, left, direction) {
    if (!jQuery.isNumeric(x) && x.match(/%]/g)) {
      x = x.split("[")[1].split("]")[0];
      if (direction == "horizontal") x = ((w + 2) * parseInt(x, 0)) / 100;
      else if (direction == "vertical") x = ((h + 2) * parseInt(x, 0)) / 100;
    } else {
      ...
      x =
        x === "left" || x === "stage_left"
          ? 0 - w - left
          : x === "right" || x === "stage_right"
          ? o.conw - left
          : x === "center" || x === "stage_center"
          ? o.conw / 2 - w / 2 - left
          : x;
      x =
        x === "top" || x === "stage_top"
          ? 0 - h - top
          : x === "bottom" || x === "stage_bottom"
          ? o.conh - top
          : x === "middle" || x === "stage_middle"
          ? o.conh / 2 - h / 2 - top
          : x;
    }
    ...
    if (customarray)
      jQuery.each(customarray, function (index, pa) {
        ...
        if (reversed && v != undefined && v.length > 0 && v.match(/\(R\)/)) {
          ...
          else if (v[0] === "[" && v[1] !== "-") v = v.replace("[", "[-");
          else if (v[0] === "-") v = v.replace("-", "");
          else if (v[0].match(/[1-9]/)) v = "-" + v;
        }

        if (v != undefined) {
          v = v.replace(/\(R\)/, "");
          if (w == "rotationX" || w == "rX")
            o.anim.rotationX = animDataTranslator(v, o.anim.rotationX) + "deg";
          if (w == "rotationY" || w == "rY")
            o.anim.rotationY = animDataTranslator(v, o.anim.rotationY) + "deg";
          if (w == "rotationZ" || w == "rZ")
            o.anim.rotation = animDataTranslator(v, o.anim.rotationZ) + "deg";
          if (w == "scaleX" || w == "sX")
            o.anim.scaleX = animDataTranslator(v, o.anim.scaleX);
          if (w == "scaleY" || w == "sY")
            o.anim.scaleY = animDataTranslator(v, o.anim.scaleY);
          if (w == "opacity" || w == "o")
            o.anim.opacity = animDataTranslator(v, o.anim.opacity);
          if (w == "skewX" || w == "skX")
            o.anim.skewX = animDataTranslator(v, o.anim.skewX);
          if (w == "skewY" || w == "skY")
            o.anim.skewY = animDataTranslator(v, o.anim.skewY);
          ...
          if (w == "transformOrigin" || w == "tO")
            o.anim.transformOrigin = v.toString();
          if (w == "transformPerspective" || w == "tP")
            o.anim.transformPerspective = parseInt(v, 0);
  ...
  var makeArray = function (obj, opt, show) {
    ...
    if (
      ...
      (obj.split(",").length > 1 || obj.split("[").length > 1)
    ) {
      obj = obj.replace("[", "");
      obj = obj.replace("]", "");
  ...
  var getcssParams = function (nc, level) {
    ...
    obj.fontSize = gp
      ? pc.data("fontsize") === undefined
        ? parseInt(pc.css("fontSize"), 0) || 0
        : pc.data("fontsize")
      : nc.data("fontsize") === undefined
      ? parseInt(nc.css("fontSize"), 0) || 0
      : nc.data("fontsize");
    obj.fontWeight = gp
      ? pc.data("fontweight") === undefined
        ? parseInt(pc.css("fontWeight"), 0) || 0
        : pc.data("fontweight")
      : nc.data("fontweight") === undefined
      ? parseInt(nc.css("fontWeight"), 0) || 0
      : nc.data("fontweight");
    obj.whiteSpace = gp
      ? pc.data("whitespace") === undefined
        ? pc.css("whitespace") || "normal"
        : pc.data("whitespace")
      : nc.data("whitespace") === undefined
      ? nc.css("whitespace") || "normal"
      : nc.data("whitespace");

    obj.lineHeight = gp
      ? pc.data("lineheight") === undefined
        ? parseInt(pc.css("lineHeight"), 0) || 0
        : pc.data("lineheight")
      : nc.data("lineheight") === undefined
      ? parseInt(nc.css("lineHeight"), 0) || 0
      : nc.data("lineheight");
    obj.letterSpacing = gp
      ? pc.data("letterspacing") === undefined
        ? parseFloat(pc.css("letterSpacing"), 0) || 0
        : pc.data("letterspacing")
      : nc.data("letterspacing") === undefined
      ? parseFloat(nc.css("letterSpacing")) || 0
      : nc.data("letterspacing");

    obj.paddingTop =
      nc.data("paddingtop") === undefined
        ? parseInt(nc.css("paddingTop"), 0) || 0
        : nc.data("paddingtop");
    obj.paddingBottom =
      nc.data("paddingbottom") === undefined
        ? parseInt(nc.css("paddingBottom"), 0) || 0
        : nc.data("paddingbottom");
    obj.paddingLeft =
      nc.data("paddingleft") === undefined
        ? parseInt(nc.css("paddingLeft"), 0) || 0
        : nc.data("paddingleft");
    obj.paddingRight =
      nc.data("paddingright") === undefined
        ? parseInt(nc.css("paddingRight"), 0) || 0
        : nc.data("paddingright");

    obj.marginTop =
      nc.data("margintop") === undefined
        ? parseInt(nc.css("marginTop"), 0) || 0
        : nc.data("margintop");
    obj.marginBottom =
      nc.data("marginbottom") === undefined
        ? parseInt(nc.css("marginBottom"), 0) || 0
        : nc.data("marginbottom");
    obj.marginLeft =
      nc.data("marginleft") === undefined
        ? parseInt(nc.css("marginLeft"), 0) || 0
        : nc.data("marginleft");
    obj.marginRight =
      nc.data("marginright") === undefined
        ? parseInt(nc.css("marginRight"), 0) || 0
        : nc.data("marginright");

    obj.borderTopWidth =
      nc.data("bordertopwidth") === undefined
        ? parseInt(nc.css("borderTopWidth"), 0) || 0
        : nc.data("bordertopwidth");
    obj.borderBottomWidth =
      nc.data("borderbottomwidth") === undefined
        ? parseInt(nc.css("borderBottomWidth"), 0) || 0
        : nc.data("borderbottomwidth");
    obj.borderLeftWidth =
      nc.data("borderleftwidth") === undefined
        ? parseInt(nc.css("borderLeftWidth"), 0) || 0
        : nc.data("borderleftwidth");
    obj.borderRightWidth =
      nc.data("borderrightwidth") === undefined
        ? parseInt(nc.css("borderRightWidth"), 0) || 0
        : nc.data("borderrightwidth");

    if (level != "rekursive") {
      obj.color =
        nc.data("color") === undefined ? "nopredefinedcolor" : nc.data("color");
      obj.whiteSpace = gp
        ? pc.data("whitespace") === undefined
          ? pc.css("whiteSpace") || "nowrap"
          : pc.data("whitespace")
        : nc.data("whitespace") === undefined
        ? nc.css("whiteSpace") || "nowrap"
        : nc.data("whitespace");

      obj.minWidth =
        nc.data("width") === undefined
          ? parseInt(nc.css("minWidth"), 0) || 0
          : nc.data("width");
      obj.minHeight =
        nc.data("height") === undefined
          ? parseInt(nc.css("minHeight"), 0) || 0
          : nc.data("height");

      if (
        nc.data("videowidth") != undefined &&
        nc.data("videoheight") != undefined
      ) {
      ...
      obj.maxWidth =
        nc.data("width") === undefined
          ? parseInt(nc.css("maxWidth"), 0) || "none"
          : nc.data("width");
      obj.maxHeight =
        nc.data("height") === undefined
          ? parseInt(nc.css("maxHeight"), 0) || "none"
          : nc.data("height");

      obj.wan =
        nc.data("wan") === undefined
          ? parseInt(nc.css("-webkit-transition"), 0) || "none"
          : nc.data("wan");
      obj.moan =
        nc.data("moan") === undefined
          ? parseInt(nc.css("-moz-animation-transition"), 0) || "none"
          : nc.data("moan");
      obj.man =
        nc.data("man") === undefined
          ? parseInt(nc.css("-ms-animation-transition"), 0) || "none"
          : nc.data("man");
      obj.ani =
        nc.data("ani") === undefined
          ? parseInt(nc.css("transition"), 0) || "none"
          : nc.data("ani");
    }

    obj.styleProps = nc.css([
      "background-color",
      "border-top-color",
      "border-bottom-color",
      "border-right-color",
  ...
  var calcCaptionResponsive = function (nc, opt, level, responsive) {
    ...
    if (!nc.hasClass("tp-splitted")) {
      ...
      var hashover =
        nc.data("transform_hover") !== undefined ||
        nc.data("style_hover") !== undefined;
      ...
      if (level != "rekursive") {
        var winw =
            obj.basealign == "slide" ? opt.ulw : opt.gridwidth[opt.curWinRange],
          winh =
            obj.basealign == "slide"
              ? opt.ulh
              : opt.gridheight[opt.curWinRange],
  ...
  var callCaptionLoops = function (el, factor) {
    ...
    if (el.hasClass("rs-pendulum")) {
      if (el.data("loop-timeline") == undefined) {
        ...
        var startdeg =
            el.data("startdeg") == undefined ? -20 : el.data("startdeg"),
          enddeg = el.data("enddeg") == undefined ? 20 : el.data("enddeg"),
          speed = el.data("speed") == undefined ? 2 : el.data("speed"),
          origin =
            el.data("origin") == undefined ? "50% 50%" : el.data("origin"),
          easing =
            el.data("easing") == undefined
        ...
        el.data("loop-timeline").append(
          new punchgs.TweenLite.fromTo(
            el,
            speed,
            { force3D: "auto", rotation: startdeg, transformOrigin: origin },
        ...
        el.data("loop-timeline").append(
          new punchgs.TweenLite.fromTo(
            el,
            speed,
            { force3D: "auto", rotation: enddeg, transformOrigin: origin },
            {
              rotation: startdeg,
              ease: easing,
              onComplete: function () {
    ...
    if (el.hasClass("rs-rotate")) {
      if (el.data("loop-timeline") == undefined) {
        el.data("loop-timeline", new punchgs.TimelineLite());
        var startdeg =
            el.data("startdeg") == undefined ? 0 : el.data("startdeg"),
    ...
    if (el.hasClass("rs-pulse")) {
      if (el.data("loop-timeline") == undefined) {
        el.data("loop-timeline", new punchgs.TimelineLite());
        var zoomstart =
            el.data("zoomstart") == undefined ? 0 : el.data("zoomstart"),
    ...
    if (el.hasClass("rs-wave")) {
      if (el.data("loop-timeline") == undefined) {
        ...
        var angle =
            el.data("angle") == undefined ? 10 : parseInt(el.data("angle"), 0),
          radius =
            el.data("radius") == undefined
              ? 10
              : parseInt(el.data("radius"), 0),
            ...
            { a: 360 },
            {
              ...
              onUpdate: function () {
                ...
                punchgs.TweenLite.to(angobj.element, 0.1, {
                  force3D: "auto",
                  x: angobj.xoffset + Math.cos(rad) * angobj.unit,
                  y: angobj.yoffset + angobj.unit * (1 - Math.sin(rad)),
                });
              },
              onComplete: function () {
                el.data("loop-timeline").restart();
  ...
  var killCaptionLoops = function (nextcaption) {
    // SOME LOOPING ANIMATION ON INTERNAL ELEMENTS
    nextcaption
      .find(".rs-pendulum, .rs-slideloop, .rs-pulse, .rs-wave")
      .each(function () {
        var el = jQuery(this);
        if (el.data("loop-timeline") != undefined) {
